...
const CFG = {
  ...
  // LANE clearance: how far a sailed passage stays off the beach. This one
  // cannot go much higher — at 6 cells (3.6 nm) the Strait of Bonifacio
  // closes and a 16 nm hop becomes a 235 nm circumnavigation of Corsica.
  clearanceCells: 5,         // keep this far off the beach (~3 nm)
  // BERTH clearance is a DIFFERENT requirement and deliberately larger. A
  // passage only has to fit through; a berth has to hold the whole hull
  // still, and the yacht is a sculptural token far larger than scale. The
  ...
  // ANISOTROPY, and it is why this is 11 rather than 10. The grid cell is
  // 0.01 DEGREES square, so it is 0.6 nm tall everywhere but only
  // 0.6 x cos(lat) nm wide — 0.46 nm in the Med, 0.38 nm in the Channel.
  // The chamfer distance counts cells, not miles, so a "10 cell" berth
  // really means 6.0 nm north-south and as little as 3.8 nm east-west up
  // at Lateral's 51N. That is under the hull's own footprint, and the
  // geometry suite caught it the moment a port went north of the Med.
  //
  // ELEVEN, not twelve, and the difference is measured rather than
  // guessed. Twelve does clear the hull, but it pushes the mean berth
  // 8.7 -> 10.5 nm offshore across all forty-four, and two suite checks
  // read that as a real change: a berth-to-berth run between two ports
  // whose berths BOTH move seaward straightens out, and Calvi-Saint Tropez
  // fell to x0.850 of the charted distance, on the floor. Eleven clears
  // the hull at Channel latitude with margin and leaves the rest of the
  // fleet where it was.
  //
  // Correcting the metric itself (target cells scaled by 1/cos(lat)) is
  // the real fix and would push every Med berth 2-4 nm further out, which
  // is a look change, not a bug fix — left as a call for the visual
  // direction rather than taken here.
  berthClearanceCells: 11,   // ~6.6 nm N-S, ~4.2 nm E-W at Channel latitude
...
// Berths MAXIMISE clearance rather than taking the first navigable cell.
// The old nearest-cell rule put every berth right on the 3 nm lane
// threshold, which is fine for a passing hull and not nearly enough for a
// stationary one. Walk outward, keep the roomiest cell seen, and stop once
// the berth target is met — so a port with open water nearby still gets a
// berth beside it, and only a port hemmed into a gulf ends up pushed far.
const anchorage = {};
for (const p of ports) {
  const px = G.x(p.lon), py = G.y(p.lat);
  let best = null, bestClr = -1;
  for (let r = 0; r <= CFG.anchorMaxCells; r++) {
    for (let dy = -r; dy <= r; dy++) {
      for (let dx = -r; dx <= r; dx++) {
        if (Math.max(Math.abs(dx), Math.abs(dy)) !== r) continue;
        const x = px + dx, y = py + dy;
        if (!navigable(x, y) || !openSea(x, y)) continue;
        const clr = seaDist[y * G.w + x];
        if (clr > bestClr) { bestClr = clr; best = [x, y]; }
      }
    }
    // Target met, and we are walking outward, so nothing closer will beat
    // it on distance-from-port. Stop rather than drifting further to sea.
    if (bestClr >= CFG.berthClearanceCells) break;
  }
  ...
  anchorage[p.name] = { cell: best, lon: G.lon(best[0]), lat: G.lat(best[1]),
                        clearanceCells: bestClr,
                        offNm: nm([p.lon, p.lat], [G.lon(best[0]), G.lat(best[1])]) };
}
{
  const short = ports.filter((p) => anchorage[p.name].clearanceCells < CFG.berthClearanceCells);
  console.log(`berths: ${ports.length - short.length}/${ports.length} reach the ` +
    `${(CFG.berthClearanceCells * CFG.grid.cell * 60).toFixed(1)} nm N-S target ` +
    `(x cos(lat) E-W — see the CFG note)` +
    (short.length ? `; hemmed in — ` + short.map((p) =>
      `${p.name} ${(anchorage[p.name].clearanceCells * CFG.grid.cell * 60).toFixed(1)}`).join(', ') : ''));
}
{
...
/* GENERATED by tools/build_terrain.mjs — do not edit by hand.
   anchorages: each port's berth — the roomiest navigable water near its
   real coordinates, targeting ${+(CFG.berthClearanceCells * CFG.grid.cell * 60).toFixed(1)} nm N-S clear of the beach (the raster
   counts square DEGREE cells, so the E-W figure is that x cos(lat) — about
   ${+(CFG.berthClearanceCells * CFG.grid.cell * 60 * Math.cos(40 * Math.PI / 180)).toFixed(1)} nm in the Med and ${+(CFG.berthClearanceCells * CFG.grid.cell * 60 * Math.cos(51 * Math.PI / 180)).toFixed(1)} nm up in the Channel) so the whole
   hull sits on water at rest, not just its centre point. Lanes only need
   ${+(CFG.clearanceCells * CFG.grid.cell * 60).toFixed(1)} nm, and cannot take more: at 3.6 nm the Strait of Bonifacio
   closes and a 16 nm hop becomes a 235 nm lap of Corsica.
...
globalThis.LEIP_SEALANES = {
  ...
  berthClearanceNm: ${+(CFG.berthClearanceCells * CFG.grid.cell * 60).toFixed(2)},
  anchorages: {
